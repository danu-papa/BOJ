 ...
 * 정점10만개. 길 100만개..
 */
public class BOJ_1647_Divde_City_Plan {
	...
	// N: 집의 개수 , M: 길의 개수
	private static int N, M, map[][], resMin, tmpres;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < M; i++) {
			...
			map[first][sec] = weight;
			map[sec][first] = weight;
		} // input end
	...
	private static void prim(int[][] tmpmap, boolean flag) {
		...
		while(!pQueue.isEmpty()) {
			...
			for(int j = 1; j <= N; j++) {
				if(!visited[j] && tmpmap[cur_Vertex][j] != 0 
						&& tmpmap[cur_Vertex][j] < minEdge[j]) {
					pQueue.offer(new Node(j,tmpmap[cur_Vertex][j]));
				}
			}
		}
	...
	/** 두 구역간 길을 끊자 */
	private static void removeRoad(int[][] tmpmap) {
		for(int i = 1; i <= N; i++) {
			for(int j = 1; j <= N; j++) {
				// 선택한 정점에서 선택안한 정점중에 길이 있다(0이 아니다)
				if(isSelected[i] && !isSelected[j] && tmpmap[i][j] != 0) {
					tmpmap[i][j] = 0;
					tmpmap[j][i] = 0;
				}
			}
